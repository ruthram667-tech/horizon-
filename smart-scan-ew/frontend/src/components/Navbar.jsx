      ...
      <div className="max-w-[1920px] mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <NavLink to="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[10px] text-sage-400 font-medium tracking-wider leading-tight">
        ...
        {!isLanding && (
          <div className="hidden md:flex items-center gap-1.5">
            <NavLink
              to="/dashboard"
              className={({ isActive }) => `nav-link text-xs font-semibold uppercase tracking-wider ${isActive ? 'active' : ''}`}
            >
        ...
        <div className="flex items-center gap-3.5">
          {!isLanding && (
            <>
              {/* Scan Status Badge */}
              <div className={`px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider border transition-all duration-300
                ${isRunning
                  ? 'bg-teal-400/10 border-teal-400/30 text-teal-400'
                  : 'bg-base-800/60 border-base-600/30 text-sage-400'
                }`}
              >
                <span className="flex items-center gap-1.5">
              ...
              {/* Connection Status */}
              <div className="flex items-center gap-1.5 bg-base-900/50 px-2.5 py-1 rounded-full border border-base-600/25">
                <div className={isConnected ? 'connection-dot-connected' : 'connection-dot-disconnected'} />
                <span className="text-[11px] text-sage-300 font-mono tracking-tight hidden sm:inline">
                  {connectionStatus}
                </span>
              </div>
              ...
              {/* Auth Info */}
              {auth && (
                <div className="hidden md:flex items-center gap-2.5 pl-3 border-l border-base-600/30">
                  <div className="text-right">
                    <div className="text-xs text-white font-semibold">{auth.username}</div>
                    <div className="text-[10px] text-teal-400/80 font-medium tracking-wide">{auth.baseRegion}</div>
                  </div>
                  <button
                    onClick={onSignOut}
